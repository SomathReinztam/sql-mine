...
for topic in topics:
    ...
    querys_parsered = response["querys_parsered"]["querys"]
    print(querys_parsered)
    print("\n\n\n\n")

    for query in querys_parsered:
        print("================= start ReActSQL\n\n")
        human_prompt = query
        messages = [SystemMessage(content=system_prompt), HumanMessage(content=human_prompt)]
        initial_state = {"messages":messages}
        k = 0
        final_step = None
        for step in ReActSQL.stream(initial_state, stream_mode='values'):
